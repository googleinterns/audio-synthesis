...
def main():
    ...
    for audio_file_path in audio_paths:
        ...
        for chunk_idx in range(processed_midi.shape[0]):
            conditioning = processed_midi[max(chunk_idx - HISTORICAL_CONDITIONING_BLOCKS, 0) :chunk_idx+1, :, :]
            conditioning = np.reshape(conditioning, (-1, 89))
            conditioning = np.pad(conditioning, [[(HISTORICAL_CONDITIONING_BLOCKS+1) * NUM_STATES_PER_CHUNK - conditioning.shape[0], 0], [0,0]])
