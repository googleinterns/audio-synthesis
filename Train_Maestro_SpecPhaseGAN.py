...
def get_waveform(spectogram):
    spectogram = np.reshape(spectogram, (128, 256, 2))
    
    return spectogram_2_waveform(spectogram, fft_length=512, frame_step=128, log_magnitude=True, instantaneous_frequency=True)
...
SpecPhaseGAN = WGAN(raw_maestro, [-1, 128, 256, 2], [-1, 128, 256, 2], generator, discriminator, Z_dim, generator_optimizer, discriminator_optimizer, generator_training_ratio=D_updates_per_g, batch_size=BATCH_SIZE, epochs=EPOCHS, checkpoint_dir=checkpoint_dir, fn_save_examples=save_examples)
